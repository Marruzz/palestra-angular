...
class DashboardController {
    ...
    try {
      ...
      if (!nome || !cognome || !data_nascita || !codice_fiscale) {
        return res.status(400).json({
          success: false,
          message:
            "Nome, cognome, data di nascita e codice fiscale sono obbligatori",
        });
      }
      ...
      if (codice_fiscale) {
        ...
        if (cfExists.length > 0) {
          return res.status(409).json({
            success: false,
            message:
              "Codice fiscale già utilizzato da un altro utente (questo utente esiste già)",
          });
        }
      }
  ...
  static async deleteUser(req, res) {
    try {
      ...
      const [result] = await pool.execute("DELETE FROM Utenti WHERE id = ?", [
        id,
      ]);
    ...
    try {
      ...
      if (!id_utente || !id_corso || !data_inizio || !durata_mesi) {
        return res.status(400).json({
          success: false,
          message:
            "Tutti i campi sono obbligatori (id_utente, id_corso, data_inizio, durata_mesi)",
        });
      }
  ...
  static async getAccesses(req, res) {
    try {
      ...
      const formattedAccesses = accesses.map((access) => ({
        ...access,
        data_ora:
          access.data_ora instanceof Date
  ...
  static async createAccess(req, res) {
    try {
      ...
      const accessData = {
        ...newAccess[0],
        data_ora:
          newAccess[0].data_ora instanceof Date
  ...
  static async updateAccess(req, res) {
    try {
      ...
      const accessData = {
        ...updatedAccess[0],
        data_ora:
          updatedAccess[0].data_ora instanceof Date
  ...
  static async getStats(req, res) {
    try {
      ...
      const [totalAccesses] = await pool.execute(
        "SELECT COUNT(*) as count FROM Ingressi"
      );
      ...
      const [totalSubscriptions] = await pool.execute(
        "SELECT COUNT(*) as count FROM Abbonamenti"
      );
      ...
      const stats = {
        ...
        corso_piu_frequentato:
          mostPopularCourse.length > 0
            ? mostPopularCourse[0].nome_corso
            : "Nessuno",
        corso_meno_frequentato:
          leastPopularCourse.length > 0
            ? leastPopularCourse[0].nome_corso
            : "Nessuno",
        tempo_medio_entrata: avgEntryTime[0].avg_time || "00:00",
        durata_media_corso: avgCourseDuration[0].avg_duration
          ? Math.round(avgCourseDuration[0].avg_duration)
          : 0,
        ultimi_accessi: recentAccesses.map((access) => ({
          ...access,
          ultimo_accesso:
            access.ultimo_accesso instanceof Date
